angular.module('myApp').controller('FriendController', function($scope) {
  ...
  $scope.randomStatus = ["old country Alabama!", "red pick-up truck", "cowboy boots", "crushing it in ping pong"];
  ...
  $scope.friends = [
   ...
   },
   {
     ...
     for(var i = 0; i < $scope.friends.length; i++) {
       $scope.index = Math.floor(Math.random() * $scope.randomStatus.length);
       $scope.newStatus = $scope.randomStatus[$scope.index];
       if($scope.friends[i].status === null) {
         $scope.friends[i].status = {
           message: ''
         };
          $scope.friends[i].status.message = $scope.newStatus;
       }
     }
    ...
    for(var i = 0; i < $scope.friends.length; i++) {
      if($scope.friends[i].current_location === null) {
        ...
        $scope.friends[i].current_location = "Missing";
      }
    }
    $scope.sortPropArray = [
      ...
      {name: 'State', value: 'current_location.city'},
      {name: 'Country', value: 'current_location.city'}
    ];
